...
class RouterOsMangleTests(TestCase):
    def test_list_managed_rules_filters_prefix_chain_and_action(self):
        api = FakeApi(
            [
                ...
                {".id": "*3", "comment": "MC:BadChain", "chain": "postrouting", "action": "mark-routing"},
                {".id": "*4", "comment": "MC:BadAction", "chain": "forward", "action": "masquerade"},
            ]
        )
    ...
    def test_changed_comment_chain_or_action_is_rejected(self):
        for row in (
            ...
            {".id": "*1", "comment": "MC:AI-Tunnel", "chain": "forward", "action": "masquerade"},
        ):
            api = FakeApi([row])
            with self.assertRaises(PermissionError):
                set_mangle_rule_disabled(api, "*1", False, settings())
